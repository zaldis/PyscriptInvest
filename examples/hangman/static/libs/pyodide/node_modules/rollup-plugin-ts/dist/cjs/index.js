...
function visitModuleDeclaration$9(_a) {
...
function visitNode$d(_a) {
    ...
    if (options.typescript.isImportDeclaration(node)) {
        ...
        return visitModuleDeclaration$9(Object.assign(Object.assign({}, options), { node }));
    }
...
function traceIdentifiersForNode(_a) {
    ...
    if (options.typescript.isBindingElement(node))
        ...
        traceIdentifiersForImportEqualsDeclaration(Object.assign(Object.assign({}, options), { node }));
...
function traceIdentifiers(_a, identifiers) {
    var { node } = _a, rest = tslib.__rest(_a, ["node"]);
    if (identifiers === void 0) { identifiers = new Set(); }
    ...
    return context => bundle => {
        ...
        if (firstEntrySourceFile != null) {
            ...
            const visitorOptions = Object.assign(Object.assign({}, options), { context,
                factory, entrySourceFilesForChunk: entrySourceFiles, otherEntrySourceFilesForChunk, sourceFile: firstEntrySourceFile, lexicalEnvironment: {
                    parent: undefined,
                    bindings: new Map()
                }, includedSourceFiles: new Set([firstEntrySourceFile.fileName]), declarationToDeconflictedBindingMap: new Map(), preservedImports: new Map(), inlinedModules: new Map(), resolveSourceFile: (fileName, from) => {
...
function generateModuleSpecifier(options) {
    ...
    if (chunkForModuleSpecifier == null) {
        ...
        return [...dependencies]
            .filter(dependency => path__default["default"].normalize(pickResolvedModule(dependency, true)) === path__default["default"].normalize(sourceFile.fileName))
    ...
    const relativePath = path__default["default"].relative(path__default["default"].dirname(chunk.paths.absolute), chunkForModuleSpecifier);
...
function statementMerger({ markAsModuleIfNeeded }) {
    return options => {
        ...
        const { mergedExports, exportedBindings } = getMergedExportDeclarationsForModules(Object.assign(Object.assign({}, options), { isTypeOnly: false }));
        const { mergedExports: mergedTypeOnlyExports } = getMergedExportDeclarationsForModules(Object.assign(Object.assign({}, options), { inputBindings: exportedBindings, isTypeOnly: true }));
...
function generateIdentifierName(module, hint) {
    return `${stringutil.camelCase(stripKnownExtension(path__default["default"].basename(module)))}${generateHintSuffix(hint)}`;
}
...
function inlineNamespaceModuleBlockTransformer({ intentToAddImportDeclaration, intentToAddModuleDeclaration }) {
    return options => {
        ...
        const transformationLog = shouldDebugSourceFile(pluginOptions.debug, sourceFile)
            ...
            : undefined;
        // Prepare some VisitorOptions
        const visitorOptions = Object.assign(Object.assign({}, options), { intentToAddImportDeclaration,
            intentToAddModuleDeclaration, childContinuation: (node) => typescript.visitEachChild(node, nextNode => visitNode$9(Object.assign(Object.assign({}, visitorOptions), { node: nextNode })), context), continuation: (node) => visitNode$9(Object.assign(Object.assign({}, visitorOptions), { node })) });
        const result = preserveMeta(typescript.visitEachChild(sourceFile, nextNode => visitorOptions.continuation(nextNode), context), sourceFile, options);
        transformationLog === null || transformationLog === void 0 ? void 0 : transformationLog.finish(result);
        fullBenchmark === null || fullBenchmark === void 0 ? void 0 : fullBenchmark.finish();
        return result;
...
function visitImportSpecifier$1(options) {
    ...
    if (exportedSymbol != null) {
        ...
        if (exportedSymbol.moduleSpecifier != null &&
            ...
            options.prependNodes(...createAliasedBinding(Object.assign(Object.assign({}, options), { node: declaration, propertyName: safePropertyName, name: contResult.name.text })));
        }
    }
        ...
        if (namedExportedSymbol.moduleSpecifier != null &&
            ...
            if ("propertyName" in namedExportedSymbol && namedExportedSymbol.propertyName != null) {
                ...
                if (safeNamedExportedSymbolPropertyName !== safeName) {
                    options.prependNodes(...createAliasedBinding(Object.assign(Object.assign({}, options), { node: declaration, propertyName: safeNamedExportedSymbolPropertyName, name: safeName })));
                }
            }
            if (contResult.propertyName != null && contResult.propertyName.text !== contResult.name.text) {
                ...
                if (safePropertyName !== safeName) {
                    options.prependNodes(...createAliasedBinding(Object.assign(Object.assign({}, options), { node: declaration, propertyName: safeName, name: safePropertyName })));
                }
            }
            return undefined;
        }
...
function moduleMerger(...transformers) {
    return options => {
        ...
        const visitorOptions = Object.assign(Object.assign(Object.assign({}, options), nodePlacementQueue), { transformers, payload: undefined, childContinuation: (node, payload) => typescript.visitEachChild(node, nextNode => nodePlacementQueue.wrapVisitResult(visitNode$8(Object.assign(Object.assign({}, visitorOptions), { payload, node: nextNode }))), context), continuation: (node, payload) => nodePlacementQueue.wrapVisitResult(visitNode$8(Object.assign(Object.assign({}, visitorOptions), { payload,
                node }))), getNameForInlinedModuleDeclaration(moduleSpecifier) {
                var _a;
            ...
            includeSourceFile(sourceFileToInclude, _a = {}) {
                ...
                const transformedSourceFile = applyTransformers({
                    visitorOptions: Object.assign(Object.assign(Object.assign({}, visitorOptions), otherOptions), { allowExports, 
...
function deconflictNode(_a) {
    ...
    if (options.typescript.isBindingElement(node)) {
        ...
        return deconflictImportEqualsDeclaration(Object.assign({ node }, options));
    }
    else if (options.typescript.isImportSpecifier(node)) {
...
function checkNode(_a) {
    ...
    if (options.typescript.isArrayBindingPattern(node)) {
        ...
        return checkImportEqualsDeclaration(Object.assign({ node, originalNode }, options));
    }
    else if (options.typescript.isExportAssignment(node)) {
...
function isReferenced(_a) {
    ...
    if (options.typescript.isExportDeclaration(options.node) ||
        ...
        if (parentNode == null || !options.typescript.isModuleBlock(parentNode)) {
            ...
            return isReferenced(Object.assign(Object.assign({}, options), { seenNodes, node: getParentNode(parentNode) }));
        }
    }
    ...
    // Compute the result
    const result = referencingNodes.length > 0 && referencingNodes.some(referencingNode => isReferenced(Object.assign(Object.assign({}, options), { seenNodes, node: referencingNode, referencedNode: options.node })));
    // Cache the result
...
function getTypeReferenceModuleFromFileName({ host, fileName }) {
    for (const typeRoot of host.getTypeRoots()) {
        if (!fileName.includes(typeRoot))
            continue;
        const base = path__default["default"].normalize(fileName.slice(typeRoot.length + 1));
...
function preparePaths({ relativeOutDir, absoluteOutDir, fileName }) {
    const absolutePath = path__default["default"].join(absoluteOutDir, fileName);
    const relativePath = path__default["default"].join(relativeOutDir, fileName);
    return {
        fileName,
...
function preNormalizeChunk(chunk) {
    return {
        modules: Object.keys(chunk.modules).map(path__default["default"].normalize),
        fileName: path__default["default"].normalize(chunk.fileName),
...
function normalizeChunk(chunk, { host, outputOptions, relativeOutDir, multiEntryModule, multiEntryFileNames }) {
    ...
    return {
        ...
        paths: preparePaths({
            ...
            absoluteOutDir: path__default["default"].join(cwd, relativeOutDir)
        }),
...
function createCommonChunk(module, code, format, chunkFileNames = `[name]-[hash].js`) {
    const name = stripKnownExtension(path__default["default"].basename(module));
...
function emitDeclarations(options) {
    ...
    const absoluteDeclarationOutDir = path__default["default"].join(cwd, relativeDeclarationOutDir);
    ...
    if (options.pluginOptions.hook.outputPath != null) {
        ...
        if (result != null) {
            virtualOutFile = preparePaths({
                fileName: path__default["default"].basename(result),
                relativeOutDir: path__default["default"].relative(cwd, path__default["default"].dirname(result)),
                absoluteOutDir: path__default["default"].dirname(result)
            });
        }
    }
    ...
    for (const chunk of normalizedChunks) {
        ...
        if (options.pluginOptions.hook.outputPath != null) {
            ...
            if (declarationResult != null) {
                declarationPaths = preparePaths({
                    fileName: path__default["default"].basename(declarationResult),
                    relativeOutDir: path__default["default"].relative(cwd, path__default["default"].dirname(declarationResult)),
                    absoluteOutDir: path__default["default"].dirname(declarationResult)
                });
            }
            if (declarationMapResult != null) {
                declarationMapPaths = {
                    ...
                    fileName: path__default["default"].basename(declarationMapResult),
                    relative: path__default["default"].join(path__default["default"].dirname(declarationPaths.relative), path__default["default"].basename(declarationMapResult)),
                    absolute: path__default["default"].join(path__default["default"].dirname(declarationPaths.absolute), path__default["default"].basename(declarationMapResult))
                };
            }
        }
        const emitFileDeclarationFilename = path__default["default"].relative(relativeOutDir, declarationPaths.relative);
        const emitFileDeclarationMapFilename = path__default["default"].relative(relativeOutDir, declarationMapPaths.relative);
        ...
        if (declarationNeedsFileSystem) {
            options.host.getFileSystem().writeFile(path__default["default"].native.normalize(declarationPaths.absolute), bundleResult.code);
        }
        // Otherwise, we can use Rollup, which is absolutely preferable
            ...
            options.pluginContext.emitFile({
                ...
                fileName: path__default["default"].native.normalize(emitFileDeclarationFilename)
            });
        }
        ...
        if (bundleResult.map != null) {
            ...
            if (declarationMapNeedsFileSystem) {
                options.host.getFileSystem().writeFile(path__default["default"].native.normalize(declarationMapPaths.absolute), bundleResult.map.toString());
            }
            // Otherwise, we can use Rollup, which is absolutely preferable
            else {
                options.pluginContext.emitFile({
                    ...
                    fileName: path__default["default"].native.normalize(emitFileDeclarationMapFilename)
                });
            }
        }
    }
...
class ModuleResolutionHost {
    ...
    clearCaches(fileName) {
        if (fileName != null) {
            ...
            this.directoryExistsCache.delete(path__default["default"].dirname(fileName));
    ...
    getFileNameDirectories() {
        if (this.currentDirectories == null) {
            this.currentDirectories = new Set([...this.getFileNames()].map(fileName => path__default["default"].dirname(fileName)));
        }
    ...
    fileExists(fileName) {
        ...
        const exists = this.files.has(fileName) || this.getFileSystem().fileExists(path__default["default"].native.normalize(fileName));
    ...
    readFile(fileName, encoding) {
        ...
        return this.getFileSystem().readFile(path__default["default"].native.normalize(fileName), encoding);
    }
    /**
    ...
    directoryExists(directoryName) {
        ...
        const result = fileNameDirectories.has(directoryName) ||
            ...
            this.getFileSystem().directoryExists(path__default["default"].native.normalize(directoryName)) ||
            this.getFileSystem().directoryExists(path__default["default"].native.normalize(absoluteDirectoryName));
     ...
     */
    realpath(p) {
        const normalized = path__default["default"].native.normalize(p);
        const fs = this.getFileSystem();
        if (fs.realpath == null)
            return normalized;
        return path__default["default"].normalize(fs.realpath(normalized));
    }
    /**
     * Gets the current directory
     */
    getCurrentDirectory() {
        return path__default["default"].normalize(this.getCwd());
    }
    /**
     * Gets all directories within the given directory path
     */
    getDirectories(directoryName) {
        return this.getFileSystem().getDirectories(path__default["default"].native.normalize(directoryName)).map(path__default["default"].normalize);
    }
}
...
function ensureModuleTransformer({ typescript, factory, sourceFile }) {
...
class CompilerHost extends ModuleResolutionHost {
    ...
    clone(compilerOptions, fileNameFilter = () => true, overrides = {}) {
        return new CompilerHost(Object.assign(Object.assign(Object.assign({}, this.options), overrides), { parsedCommandLineResult: Object.assign(Object.assign({}, this.options.parsedCommandLineResult), { parsedCommandLine: Object.assign(Object.assign({}, this.getParsedCommandLine()), { fileNames: this.getParsedCommandLine().fileNames.filter(fileNameFilter), options: Object.assign(Object.assign({}, this.getCompilationSettings()), compilerOptions) }) }) }), this.printer, new Map([...this.sourceFiles.entries()].filter(([p]) => fileNameFilter(p))), new Map([...this.transformerDiagnostics.entries()].filter(([p]) => fileNameFilter(p))), new Map([...this.fileToVersionMap.entries()].filter(([p]) => fileNameFilter(p))), new Map([...this.sourceFileToDependenciesMap.entries()].filter(([p]) => fileNameFilter(p))), new Map([...this.files.entries()].filter(([p]) => fileNameFilter(p))));
    }
    getSourceFile(fileName, languageVersion = this.getScriptTarget()) {
        var _a;
        const absoluteFileName = path__default["default"].includeDriveLetter(isTypeScriptLib(fileName) ? path__default["default"].join(this.getDefaultLibLocation(), fileName) : ensureAbsolute(this.getCwd(), fileName));
    ...
    getDefaultLibLocation() {
        return path__default["default"].dirname(this.getTypescript().getDefaultLibFilePath(this.getCompilationSettings()));
    }
    /**
    ...
    readDirectory(p, extensions, exclude, include, depth) {
        return this.getFileSystem().readDirectory(path__default["default"].native.normalize(p), extensions, exclude, include, depth).map(path__default["default"].normalize);
    }
    ...
    addDefaultFileNames() {
        this.getParsedCommandLine().fileNames.forEach(file => {
            ...
            if (!this.getFilter()(path__default["default"].normalize(fileName)))
                return;
...
function emitBuildInfo(options) {
    ...
    const emitFile = path__default["default"].join(path__default["default"].relative(relativeOutDir, outputPathCandidate));
    // Rollup does not allow emitting files outside of the root of the whatever 'dist' directory that has been provided.
    ...
    if (needsFileSystem) {
        options.host.getFileSystem().writeFile(path__default["default"].native.normalize(outputPathCandidate), buildInfo.text);
    }
    // Otherwise, we can use Rollup, which is absolutely preferable
    else {
        options.pluginContext.emitFile({
            ...
            fileName: path__default["default"].native.normalize(emitFile)
        });
    }
}
...
async function loadBabel() {
    return (babelModule !== null && babelModule !== void 0 ? babelModule : (babelModule = await loadModules("babel", "@babel/core", ["@babel/runtime", "@babel/plugin-transform-runtime", "@babel/preset-env"])));
}
...
async function loadModules(context, coreModule, subModules) {
    ...
    const results = await Promise.all(moduleNames
        .map(async (moduleName) => (function (t) { return Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(t)); }); })(moduleName))
...
function getBabelConfig({ babel, babelConfig, cwd, forcedOptions = {}, defaultOptions = {}, browserslist, phase, hook }) {
    return (filename) => {
        ...
        const combined = Object.assign(Object.assign(Object.assign(Object.assign({}, otherDefaultOptions), options), otherForcedOptions), { presets: combineConfigItems(((_a = options.presets) !== null && _a !== void 0 ? _a : []), defaultPresets == null ? undefined : (_c = (_b = babel.loadPartialConfig(Object.assign({ presets: defaultPresets }, configFileOption))) === null || _b === void 0 ? void 0 : _b.options.presets) !== null && _c !== void 0 ? _c : undefined, forcedPresets == null
                ? undefined
                : (_e = (_d = babel.loadPartialConfig(Object.assign({ presets: forcedPresets }, configFileOption))) === null || _d === void 0 ? void 0 : _d.options.presets) !== null && _e !== void 0 ? _e : undefined, phase === "chunk"), plugins: combineConfigItems(((_f = options.plugins) !== null && _f !== void 0 ? _f : []), defaultPlugins == null ? undefined : (_h = (_g = babel.loadPartialConfig(Object.assign({ plugins: defaultPlugins }, configFileOption))) === null || _g === void 0 ? void 0 : _g.options.plugins) !== null && _h !== void 0 ? _h : undefined, forcedPlugins == null
                ? undefined
                : (_k = (_j = babel.loadPartialConfig(Object.assign({ plugins: forcedPlugins }, configFileOption))) === null || _j === void 0 ? void 0 : _j.options.plugins) !== null && _k !== void 0 ? _k : undefined, phase === "chunk") });
        ...
        const loadedOptions = (_o = babel.loadOptions(Object.assign(Object.assign(Object.assign({}, combinedOptionsAfterHook), { filename }), configFileOption))) !== null && _o !== void 0 ? _o : undefined;
...
function readConfig(config, cwd, fileSystem) {
    if (config == null) {
        const absoluteConfig = path__default["default"].normalize(path__default["default"].join(cwd, `.swcrc`));
        ...
        const absoluteConfig = path__default["default"].normalize(path__default["default"].isAbsolute(config) ? config : path__default["default"].join(cwd, config));
        if (!fileSystem.fileExists(absoluteConfig)) {
 ...
 */
function getSwcConfigFactory({ fileSystem, swcConfig, cwd, browserslist, ecmaVersion, phase, typescript, hook }) {
    ...
    return filename => {
        ...
        const config = Object.assign(Object.assign({}, inputConfig), { module: Object.assign(Object.assign({}, inputConfig.module), FORCED_SWC_MODULE_OPTIONS), jsc: Object.assign(Object.assign(Object.assign(Object.assign({ 
                // Loose breaks things such as spreading an iterable that isn't an array
                loose: false }, inputConfig.jsc), { parser: Object.assign(Object.assign({ syntax: "ecmascript" }, (((_b = (_a = inputConfig.jsc) === null || _a === void 0 ? void 0 : _a.parser) === null || _b === void 0 ? void 0 : _b.syntax) === "typescript" ? {} : { jsx: false })), (_c = inputConfig.jsc) === null || _c === void 0 ? void 0 : _c.parser) }), (browserslist == null && ecmaVersion != null
                ? {
                    target: getEcmaVersionForScriptTarget(ecmaVersion, typescript)
                }
                : {})), FORCED_SWC_JSC_OPTIONS), env: Object.assign({}, (browserslist == null
                ? inputConfig.env
                : Object.assign({ 
                    // Loose breaks things such as spreading an iterable that isn't an array
                    loose: false, shippedProposals: true, targets: browserslist }, inputConfig.env))), cwd,
            filename,
            minify, swcrc: false, configFile: false, root: cwd, 
            // Always produce sourcemaps. Rollup will be the decider of what to do with them.
            sourceMaps: true, 
            // Always use the cwd provided to the plugin
            ...
            // Always parse things as modules. Rollup will then decide what to do based on the output format
            isModule: true });
        return (_d = hook === null || hook === void 0 ? void 0 : hook(config, filename, phase)) !== null && _d !== void 0 ? _d : config;
    };
}
...
function typescriptRollupPlugin(pluginInputOptions = {}) {
    ...
    const filter = (id) => !isSwcHelper(id) && (internalFilter(id) || internalFilter(path__default["default"].normalize(id)) || internalFilter(path__default["default"].native.normalize(id)));
    /**
    ...
    const addAndEmitFile = (fileName, text, dependencyCb) => {
    ...
    return {
        ...
        async options(options) {
            ...
            switch (pluginOptions.transpiler) {
                ...
                case "swc": {
                    ...
                    swcConfigFileFactory = getSwcConfigFactory(Object.assign(Object.assign({}, sharedSwcConfigFactoryOptions), { phase: "file" }));
                    swcConfigChunkFactory = getSwcConfigFactory(Object.assign(Object.assign({}, sharedSwcConfigFactoryOptions), { phase: "chunk" }));
                    break;
                }
            }
        ...
        async renderChunk(code, chunk, outputOptions) {
            ...
            switch (pluginOptions.transpiler) {
                case "babel": {
                    ...
                    const transpilationResult = await babel.transformAsync(updatedCode, Object.assign(Object.assign(Object.assign({}, config), { filenameRelative: ensureRelative(cwd, chunk.fileName) }), (updatedMap == null
                        ? {}
                        : {
                ...
                case "swc": {
                    ...
                    const updatedCode = updatedSourceDescription != null ? updatedSourceDescription.code : code;
                    const updatedMap = updatedSourceDescription != null ? updatedSourceDescription.map : undefined;
                    const transpilationResult = await swc.transform(updatedCode, Object.assign(Object.assign({}, config), (updatedMap == null
                        ? {}
                        : {
        ...
        async transform(code, file) {
            ...
            if (isMultiEntryModule(normalizedFile, MULTI_ENTRY_MODULE)) {
                MULTI_ENTRY_FILE_NAMES = new Set(stringutil.matchAll(code, /(import|export)\s*(\*\s*from\s*)?["'`]([^"'`]*)["'`]/).map(([, , , p]) => path__default["default"].normalize(p.replace(/\\\\/g, "\\"))));
                return undefined;
            }
            ...
            switch (pluginOptions.transpiler) {
                case "babel": {
                    ...
                    if (isSupportedByCompilerHost) {
                        sourceDescription = (_b = addAndEmitFile(normalizedFile, sourceDescription.code, dependency => this.addWatchFile(dependency))) !== null && _b !== void 0 ? _b : sourceDescription;
                    }
                    const transpilationResult = await babel.transformAsync(sourceDescription.code, Object.assign(Object.assign({}, babelConfigResult.config), { filenameRelative: ensureRelative(cwd, normalizedFile), inputSourceMap: typeof sourceDescription.map === "string" ? JSON.parse(sourceDescription.map) : sourceDescription.map }));
                    if (transpilationResult == null || transpilationResult.code == null) {
        ...
        resolveId(id, parent) {
            ...
            return pickedResolveResult == null ? null : path__default["default"].native.normalize(pickedResolveResult);
        },
        /**
        ...
        load(id) {
            ...
            if (isRegeneratorRuntime(path__default["default"].normalize(id))) {
        ...
        generateBundle(outputOptions, bundle) {
            ...
            if (inputCache != null) {
                for (const module of inputCache.modules) {
                    const normalizedFile = path__default["default"].normalize(module.id);
            ...
            for (const file of Object.values(bundle)) {
                if (!("fileName" in file))
                    continue;
                const normalizedFileName = path__default["default"].normalize(file.fileName);
